...
import { createMockSession, createMockNotesRepository } from '@/test-utils';
...
function FullIntegrationTestComponent() {
  ...
  return (
    <div>
      ...
      <span data-testid="auth-is-authenticated">{auth.isAuthenticated.toString()}</span>
      <span data-testid="auth-is-loading">{auth.isLoading.toString()}</span>

      {/* Repository availability */}
      <span data-testid="repo-available">{!!repository.notesRepository ? 'true' : 'false'}</span>
...
function AuthFlowTestComponent() {
  ...
  const testAuthenticatedOperation = async () => {
    try {
      ...
      setOperationResults({ success: true, notesCount: notes.items?.length || 0 });
    } catch (error) {
      setOperationResults({ 
        success: false, 
  ...
  return (
    <div>
      ...
      <span data-testid="operation-success">{operationResults.success ? 'true' : 'false'}</span>
      <span data-testid="operation-notes-count">{operationResults.notesCount || 0}</span>
      <span data-testid="operation-error">{operationResults.error || 'null'}</span>
      <button 
...
describe('Cross-Provider Integration Tests', () => {
  ...
  describe('Full provider stack integration', () => {
    it('should integrate auth, repository, query, and note providers successfully', async () => {
      ...
      const { getByTestId } = render(
        <FullProviderWrapper session={mockSession} noteId={mockNote.id} mockRepo={mockNotesRepository}>
      ...
      expect(getByTestId('auth-token')).toHaveTextContent(mockSession.accessToken);
    ...
    it('should handle unauthenticated state across all providers', async () => {
      ...
      const { getByTestId } = render(
        <FullProviderWrapper noteId={mockNote.id} mockRepo={mockNotesRepository}>
          <FullIntegrationTestComponent />
        </FullProviderWrapper>
      );
  ...
  describe('Auth state changes across providers', () => {
    it('should propagate auth state changes to all providers', async () => {
      ...
      rerender(
        <FullProviderWrapper session={mockSession} mockRepo={mockNotesRepository}>
          <FullIntegrationTestComponent />
        </FullProviderWrapper>
      );
  ...
  describe('Error handling across providers', () => {
    it('should handle repository errors gracefully while maintaining auth state', async () => {
      ...
      mockNotesRepository.getNoteById.mockRejectedValue(new Error('Note not found'));

      const { getByTestId } = render(
        <FullProviderWrapper session={mockSession} noteId={999} mockRepo={mockNotesRepository}>
          <FullIntegrationTestComponent />
        </FullProviderWrapper>
      );
        ...
        return (
          <div>
            ...
            <span data-testid="hierarchy-working">
              {auth.isAuthenticated && repository.notesRepository && note.id ? 'true' : 'false'}
            </span>
          </div>
        );
      }
  ...
  describe('Real-world usage patterns', () => {
    it('should handle typical user authentication flow', async () => {
      ...
      expect(parseInt(getByTestId('operation-notes-count').textContent || '0')).toBeGreaterThan(0);
      expect(mockNotesRepository.setAuthToken).toHaveBeenCalledWith(mockSession.accessToken);
